  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.undo, color: Colors.white70), onPressed: () {}),
          IconButton(icon: const Icon(Icons.redo, color: Colors.white70), onPressed: () {}),
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            // 2. PANEL BAWAH
            Obx(() => Container(
                  height: 260, // Tinggi diturunkan sedikit karena slidernya lebih sedikit
  ...
  Widget _buildActiveSliders(EditorController controller) {
    switch (controller.currentSubMenu.value) {
      case EditSubMenu.light:
        return Column(
          children: [
            _buildSliderRow('Exposure', controller.exposure, -100, 100),
            _buildSliderRow('Contrast', controller.contrast, -100, 100),
          ],
        );
        ...
        return Column(
          children: [
            Row(
              ...
              children: [
                OutlinedButton(
                  onPressed: () => controller.isBlackAndWhite.toggle(), 
                  child: Obx(() => Text(controller.isBlackAndWhite.value ? 'B&W (On)' : 'B&W (Off)'))
                ),
              ],
            ),
            const SizedBox(height: 16),
            _buildSliderRow('Temp', controller.temperature, 2000, 10000, suffix: 'K'),
            _buildSliderRow('Saturation', controller.saturation, -100, 100),
          ],
        );
    }
  }

  Widget _buildSliderRow(String label, RxDouble rxValue, double min, double max, {String suffix = ''}) {
    return Padding(
